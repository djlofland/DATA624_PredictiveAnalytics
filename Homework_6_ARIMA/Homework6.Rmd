...
(1-2*B+B^2) * y_t = (\eta_t + \theta_1*B*\epsilon_t)\\
y_t - 2*y_t*B + y_t*B^2 = (\eta_t + \theta_1*B*\epsilon_t)\\
y_t = 2*y_t*B - y_t*B^2 + \eta_t + \theta_1*B*\epsilon_t\\
y_t = 2*y_{t-1} - y_{t-2} + \eta_t + \theta_1 * \epsilon_{t-1}\\
y_{T+1|T} = 2*y_{T} - y_{T-1} + \theta_1 * \epsilon_{T}$$

> I give up ... sorry - calculating by hand isn't going well and I need to move on.  Note: I did spend quite a bit of time trying to fingure this out!  I don't know how to get $\epsilon_T$.   I know $\theta$ from the model (-0.8995) and the last observed value in the series is `r wmurders[54]`.  
